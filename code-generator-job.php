...
<div class="btn-group pull-right page-menu">
    ...
    <div rel="open-modal" data-script="codegenerator_job/add_job.php?id=<?=$id?>" class="btn btn-success" title="Add add ons job" tooltip>
        <i class="icon-plus"></i>
    </div>
</div>
...
<h1 class="page-title"><i class="icon-question"></i>Code generator job</h1>
          ...
          <table class="data-table-header" cellpadding="0" cellspacing="0" border="0">
            <tr valign='center' >
              <td id="headername" name="headername">
                Codegenerator Job list
              </td>
              <td style="text-align:right;padding-right:10px;padding-top:5px;" id="searchbox" name="searchbox">
              <?php
              $account_id=$_SESSION['ao_accountid'];
              ?>
              <input type="button" value="Set Order" class="btn btn-blue pull-right"  data-script="codegenerator_job_order.php?id=<?=$id?>" rel="open-modal" ></input>
              </td>
            </tr>
          </table>
        </td>
      ...
      <tr>
        <td>
          ...
          <script type='text/javascript'>
            Request.make('<?=AJAX_DIR?>/codegenerator_job/get_joblist.php?id=<?=$id?>', 'knowledgebasecontainer', true, true);
          </script>
        </td>
      </tr>
    </table>
    ...
    function search_checklistjob(n)
    {
        ...
        if (this.readyState == 4 && this.status == 200) 
        {
          ...
          else
          {
              ...
              for(var i=0;i<myObj.joblist.length;i++)
              {
                ...
                if(i%2==0)
                  ...
                  p+="<td width='60%'>";
                  p+="<a href='javascript: Request.make(\"includes/ajax/codegenerator_job/get_joblist.php?";
                  ...
                  p+="<div style='cursor: pointer;float: left;'  rel='open-modal' data-script='checklist_job/edit_job.php?checklist_job_id="+myObj.joblist[i].id;
                  p+="' class=''  tooltip='Edit'><i class='icon-pencil'></i></div>";
                  p+="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:if(confirm(\"Are you sure?\")) Request.make(\"includes/ajax/codegenerator_job/get_joblist.php?id=<?=$id?>&codegenerator_job_id=";
          ...
          xmlhttp.open("GET", "<?=AJAX_DIR?>/checklist_job/get_job_searched_list.php?id=<?=$id?>&search_key="+n, true);
          xmlhttp.send();
