...
<table border="0" class="schedulecontainer" width="100%">
    ...
    $sql = "SELECT t2.salesman,CONCAT(t3.fname, ' ', t3.lname) as customer_name,t3.rate,SUM(TIMESTAMPDIFF(MINUTE,t1.start_time,t1.end_time)) as total_time,FORMAT(SUM((TIMESTAMPDIFF(MINUTE,t1.start_time,t1.end_time)/60)*IF(t4.rate IS NULL,0,t4.rate)),2) as total_bill,IF(p.amnt IS NULL ,0,p.amnt) as total_paid
