...
if(!empty($delete_flag))
{
    ...
    $sql = "update tbl_ventcalculator_job set is_deleted=1 where tbl_ventcalculator_job_id=".$ventcalculator_job_id;
           DBUtil::query($sql);
  $ventcalculator_job_id=null;         
}
if(empty($ventcalculator_job_id) ){
      ...
      <table border="0" cellspacing="0" cellpadding=2 width="100%" class="infocontainernopadding" id="resulttable" name="resulttable">
       <?php
        $ac_id = $_SESSION['ao_accountid'];
         ...
         $sql = "SELECT t1.tbl_ventcalculator_job_id, t1.name,t1.order_num,t2.ventcalculator_name from tbl_ventcalculator_job as t1
        left join tbl_ventcalculator as t2 on t2.ventcalculator_id=t1.ventcalculator_id  
        where t1.account_id=".$ac_id." and t1.ventcalculator_id='".$id."' order by t1.order_num desc";
        ...
        if(empty($checklist))
        {
        	$not_found_header="No Vent job found";
        }
        ?>
  ...
  foreach($checklist as $row)
  {
    ...
    <table width="100%">
     <tr class='<?php echo $class; ?>' valign='top'>
          ...
          <td width="60%">    
            <a href='javascript: Request.make("includes/ajax/ventcalculator_job/get_joblist.php?id=<?=$id?>&ventcalculator_job_id=<?php echo $row['tbl_ventcalculator_job_id']; ?>","knowledgebasecontainer","yes","yes");' class='basiclink'> <?php echo $row['name']; ?></a>
          </td>
          <td width="20%" <?=$display?>>
          ...
          <td <?=$display?>  width="20%">
              ...
              <div title="Edit" style="cursor: pointer;float: left;" rel="open-modal" data-script="ventcalculator_job/edit_job.php?id=<?=$id?>&ventcalculator_job_id=<?php echo $row['tbl_ventcalculator_job_id']; ?>" class=""  tooltip="Edit">
              <i class="icon-pencil"></i>
              </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

              <a title="Delete"  href='javascript:if(confirm("Are you sure?")) Request.make("includes/ajax/ventcalculator_job/get_joblist.php?id=<?=$id?>&ventcalculator_job_id=<?php echo $row['tbl_ventcalculator_job_id']; ?>&delete_flag=1","knowledgebasecontainer","yes","yes");' tooltip="Delete"><i class="icon-trash" style="color:red" > </i></a>
          </td>
...
<tr><td class="infofooter"><a href="<?=ROOT_DIR?>/vent-calculator.php" class="basiclink"><i class="icon-double-angle-left"></i>&nbsp;Back</a></td></tr>
    ...
    $(document).ready(function(){
     ...
     headername.innerHTML="Vent Calculator job list";
...
else
{
  ...
  if(isset($attachment_name) )
  {
       ...
       // $file_dir='https://workflowdev.xactbid.com/workflow/docs/Knowledgebase/' ;
        $file_dir=ROOT_DIR.'/docs/ventcalculator/';
      ...
      <a href='javascript: Request.make("includes/ajax/ventcalculator_job/get_joblist.php?id=<?=$id?>&ventcalculator_job_id=<?=$tbl_ventcalculator_job_id?>","knowledgebasecontainer","yes","yes");' class='basiclink'>
