...
$ventcalculator_job_id=RequestUtil::get('ventcalculator_job_id');
...
{
    $sql = "select attachment,attachment_desc from tbl_ventcalculator_job where tbl_ventcalculator_job_id=".$ventcalculator_job_id;
    ...
    $sql = "update tbl_ventcalculator_job set attachment='". $new_attachment."',attachment_desc='". $new_file_descriptions."' where tbl_ventcalculator_job_id=".$ventcalculator_job_id;
...
if(RequestUtil::get("submit"))
{
      ...
      for($i=0;$i<count($_FILES['myFile']['name']);$i++)
      {
          ...
          $new_path = DOCUMENTS_PATH. '/ventcalculator/' . $new_filename;
      ...
      if(!count($errors)) 
      {
          $sql = "update tbl_ventcalculator_job set name='".$name."',ventcalculator_id=".$ventcalculator_id.", description='".$description."',attachment='".$attachment_attachment."',attachment_desc='".$attachment_file_description."'  where tbl_ventcalculator_job_id=".$ventcalculator_job_id;
             DBUtil::query($sql);
      ?>
...
<?php
if(RequestUtil::get("ventcalculator_job_id")) 
{
$ventcalculator_job_id = RequestUtil::get('ventcalculator_job_id');

$sql = "select tbl_ventcalculator_job_id,ventcalculator_id,name,description,attachment,attachment_desc from tbl_ventcalculator_job where tbl_ventcalculator_job_id=".$ventcalculator_job_id;
//echo $sql;
$res = DBUtil::query($sql);


//$res = DBUtil::query($sql);
list($ventcalculator_job_id, $ventcalculator_id,$name,$description,$attachment,$file_descriptions)=mysqli_fetch_row($res);
$file_dir=DOCUMENTS_PATH. '/ventcalculator/' ;
$attachment_array=explode("##",$attachment);
$file_descriptions_array=explode("##",$file_descriptions);
...
?>
<form method="post" name="knowledgebase" action="?id=<?=$id?>&&ventcalculator_job_id=<?php echo $_REQUEST['ventcalculator_job_id']; ?>" enctype='multipart/form-data'>
<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
    <tr>
        <td>
            <table class="data-table-header" cellpadding="0" cellspacing="0" border="0">
                <tr valign="center">
                    <td>Update Vent Calculator Job</td>
                    <td align="right">
            ...
            <table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
                <tr>
                    ...
                    <td class="listrownoborder">
                        <input type="hidden" id="ventcalculator_job_id" name="ventcalculator_job_id" value="<?=$ventcalculator_job_id?>" />
                 ...
                 <tr>
                    ...
                    <td class="listrownoborder">
                        ...
                        {
                        ?>
                          <option value="<?= $row['ventcalculator_id'] ?>" <?php echo ($row['ventcalculator_id']==$ventcalculator_id)?'selected="selected"':''?>><?=$row['ventcalculator_name']?></option>
                        <?php
                        }
                        ?>
                      </select>
...
for($i=0;$i<count($attachment_array);$i++)
{
    if(!empty($attachment_array[$i]))
    {
     ...
     // echo "hh=".strpos($attachment_array[$i],"_");
      ...
      $display_string.="<td width='10%'>".($i+1).".</td><td>".$file_display_name."</td><td><input id='file_description_".$i."' name='file_description_".$i."' type='text' value='".$file_description."'></input></td><td><a title='Update attachment description' href='#' onclick='save_attachment(\"".$ventcalculator_job_id."\",\"". $file_name."\",\"".$i."\")'><i class='icon-save'  > </i></a>";
      $display_string.="&nbsp;&nbsp&nbsp;&nbsp;<a href='javascript:if(confirm(\"Are you sure?\")) window.location.href=\"".AJAX_DIR."/ventcalculator_job/edit_job.php?id=<?=$id?>&ventcalculator_job_id=".$ventcalculator_job_id."&attachment_name=".$file_name."&delete_flag=1\";' title=\"Delete attachment\"><i class=\"icon-trash\" style=\"color:red\"> </i></a></td>";
...
function save_attachment(kid,att_name,id)
{
   ...
   var file_description=document.getElementById(''+file_desc_textbox).value;
    ...
    window.location.href="edit_job.php?ventcalculator_job_id="+kid+"&attachment_name="+att_name+"&file_desc="+file_description;
    }catch(e){alert(e);}
}
