...
if(RequestUtil::get('update')) {    
    ...
    foreach($input as $checklist_id){
        $sql = "update tbl_checklist set order_num = ".$count." where checklist_id = ".$checklist_id;
...
<form method="post">
    ...
    <table style="padding:30px;" cellpadding="10" id="job_stage_table" class="table-striped table-hover">
        ...
        <tr>
            ...
            <td><?=$stage['checklist_name']?><input type="hidden" name="checklists[]" value="<?=$stage['checklist_id']?>"></td>            
        </tr>
        <?php } ?>        
    </table>
     <input type="submit" name="update" value="Update">
</form>
<p><i>* Use Drag and Drop to change the order of checklist and then, click Update</i></p>
