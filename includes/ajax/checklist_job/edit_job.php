...
$checklist_job_id=RequestUtil::get('checklist_job_id');
$name = RequestUtil::get('name');
$checklist_id = RequestUtil::get('type');
...
{
    $sql = "select attachment,attachment_desc from tbl_checklist_job where tbl_checklist_job_id=".$checklist_job_id;
    ...
    $sql = "update tbl_checklist_job set attachment='". $new_attachment."',attachment_desc='". $new_file_descriptions."' where tbl_checklist_job_id=".$checklist_job_id;
...
if(RequestUtil::get("submit"))
{
      ...
      for($i=0;$i<count($_FILES['myFile']['name']);$i++)
      {
          ...
          $new_path = DOCUMENTS_PATH. '/checklist/' . $new_filename;
      ...
      if(!count($errors)) 
      {
          $sql = "update tbl_checklist_job set name='".$name."',checklist_id=".$checklist_id.", description='".$description."',attachment='".$attachment_attachment."',attachment_desc='".$attachment_file_description."'  where tbl_checklist_job_id=".$checklist_job_id;
             DBUtil::query($sql);
      ?>
...
<?php
if(RequestUtil::get("checklist_job_id")) 
{
$checklist_job_id = RequestUtil::get('checklist_job_id');

$sql = "select tbl_checklist_job_id,checklist_id,name,description,attachment,attachment_desc from tbl_checklist_job where tbl_checklist_job_id=".$checklist_job_id;
//echo $sql;
$res = DBUtil::query($sql);


//$res = DBUtil::query($sql);
list($checklist_job_id, $checklist_id,$name,$description,$attachment,$file_descriptions)=mysqli_fetch_row($res);
$file_dir=DOCUMENTS_PATH. '/checklist/' ;
$attachment_array=explode("##",$attachment);
$file_descriptions_array=explode("##",$file_descriptions);
            ...
            <table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
                <tr>
                    ...
                    <td class="listrownoborder">
                        <input type="hidden" id="checklist_job_id" name="checklist_job_id" value="<?=$checklist_job_id?>" />
                 ...
                 <tr>
                    ...
                    <td class="listrownoborder">
                        <?php
                        $sql = "SELECT checklist_id, checklist_name from tbl_checklist
                        ...
                        {
                        ?>
                          <option value="<?= $row['checklist_id'] ?>" <?php echo ($row['checklist_id']==$checklist_id)?'selected="selected"':''?>><?=$row['checklist_name']?></option>
                        <?php
                        }
                        ?>
                      </select>
...
for($i=0;$i<count($attachment_array);$i++)
{
    if(!empty($attachment_array[$i]))
    {
     ...
     // echo "hh=".strpos($attachment_array[$i],"_");
      ...
      $display_string.="<td width='10%'>".($i+1).".</td><td>".$file_display_name."</td><td><input id='file_description_".$i."' name='file_description_".$i."' type='text' value='".$file_description."'></input></td><td><a title='Update attachment description' href='#' onclick='save_attachment(\"".$checklist_job_id."\",\"". $file_name."\",\"".$i."\")'><i class='icon-save'  > </i></a>";
      $display_string.="&nbsp;&nbsp&nbsp;&nbsp;<a href='javascript:if(confirm(\"Are you sure?\")) window.location.href=\"".AJAX_DIR."/checklist_job/edit_job.php?id=<?=$id?>&checklist_job_id=".$checklist_job_id."&attachment_name=".$file_name."&delete_flag=1\";' title=\"Delete attachment\"><i class=\"icon-trash\" style=\"color:red\"> </i></a></td>";
...
function save_attachment(kid,att_name,id)
{
   ...
   var file_description=document.getElementById(''+file_desc_textbox).value;
    ...
    window.location.href="edit_job.php?checklist_job_id="+kid+"&attachment_name="+att_name+"&file_desc="+file_description;
    }catch(e){alert(e);}
}
