...
if(!empty($delete_flag))
{
  ...
  $sql = "update tbl_checklist_job set is_deleted=1 where tbl_checklist_job_id=".$checklist_job_id;
           DBUtil::query($sql);
  $checklist_job_id=null;         
}
if(empty($checklist_job_id) ){
    ...
    <td>
      <table border="0" cellspacing="0" cellpadding=2 width="100%" class="infocontainernopadding" id="resulttable" name="resulttable">
       <?php
        ...
        where t1.account_id=".$ac_id." and t1.checklist_id='".$id."' and t1.is_deleted='0' order by t1.order_num,t1.tbl_checklist_job_id";
  ...
  foreach($checklist as $row)
  {
    ...
    <table width="100%">
     <tr class='<?php echo $class; ?>' valign='top'>
          ...
          <td width="60%">    
            <a href='javascript: Request.make("includes/ajax/checklist_job/get_joblist.php?id=<?=$id?>&checklist_job_id=<?php echo $row['tbl_checklist_job_id']; ?>","knowledgebasecontainer","yes","yes");' class='basiclink'> <?php echo $row['name']; ?></a>
          </td>
          <td width="20%" <?=$display?>>
          ...
          <td <?=$display?>  width="20%">
              ...
              <div title="Edit" style="cursor: pointer;float: left;" rel="open-modal" data-script="checklist_job/edit_job.php?id=<?=$id?>&checklist_job_id=<?php echo $row['tbl_checklist_job_id']; ?>" class=""  tooltip="Edit">
              <i class="icon-pencil"></i>
              </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

              <a title="Delete"  href='javascript:if(confirm("Are you sure?")) Request.make("includes/ajax/checklist_job/get_joblist.php?id=<?=$id?>&checklist_job_id=<?php echo $row['tbl_checklist_job_id']; ?>&delete_flag=1","knowledgebasecontainer","yes","yes");' tooltip="Delete"><i class="icon-trash" style="color:red" > </i></a>
          </td>
...
<tr><td class="infofooter"><a href="<?=ROOT_DIR?>/check-list.php" class="basiclink"><i class="icon-double-angle-left"></i>&nbsp;Back</a></td></tr>
    ...
    $(document).ready(function(){
     ...
     headername.innerHTML="Checklist Job";
...
else
{
  ...
  if(isset($attachment_name) )
  {
       ...
       // $file_dir='https://workflowdev.xactbid.com/workflow/docs/Knowledgebase/' ;
        $file_dir=ROOT_DIR.'/docs/checklist/';
      ...
      <a href='javascript: Request.make("includes/ajax/checklist_job/get_joblist.php?id=<?=$id?>&checklist_job_id=<?=$tbl_checklist_job_id?>","knowledgebasecontainer","yes","yes");' class='basiclink'>
...
$res = DBUtil::query($sql);
list($tbl_checklist_job_id,$checklist_id,$name,$attachment,$description,$attachment_desc)=mysqli_fetch_row($res);
//$file_dir=DOCUMENTS_PATH. '/Knowledgebase/' ;
$file_dir='/workflow/docs/checklist/' ;
...
<tr>
        ...
        <tr>
          <td class='infofooter' colspan=2>
			<a href='javascript: Request.make("includes/ajax/checklist_job/get_joblist.php?id=<?=$id?>","knowledgebasecontainer","yes","yes");' class='basiclink'>
				<i class="icon-double-angle-left"></i>&nbsp;Back
			</a>
          </td>
        </tr>
