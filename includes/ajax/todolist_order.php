...
if(RequestUtil::get('update')) {    
    $input = RequestUtil::get('todolists');
    $count = 1;
    foreach($input as $todolist_id){
        $sql = "update tbl_todolist set order_num = ".$count." where todolist_id = ".$todolist_id;
...
<form method="post">
    ...
    <table style="padding:30px;" cellpadding="10" id="job_stage_table" class="table-striped table-hover">
        ...
        <tr>
            ...
            <td><?=$stage['todolist_name']?><input type="hidden" name="todolists[]" value="<?=$stage['todolist_id']?>"></td>            
        </tr>
        <?php } ?>        
    </table>
