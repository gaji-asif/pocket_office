...
class DBUtil extends AssureUtil {
    ...
    public static function connect($databaseName, $log = TRUE) {
       ...
       self::$link = @mysqli_connect($databaseCreds['host'], $databaseCreds['user'], $databaseCreds['password']);
        ...
        //select db
        mysqli_select_db(self::$link,$databaseName);
...
//        R::setup("mysql:host={$databaseCreds['host']};dbname=$databaseName", $databaseCreds['user'], $databaseCreds['password']);
    ...
    public static function query($sql) {
        ...
        if(is_array($gdb))
        {
                $db_name=$gdb['assure']['database'];
			   mysqli_select_db(self::$link,$db_name);

        }
        /*-----------------------------------------*/

        $results = mysqli_query(self::$link,$sql)or mysqli_error();
...
//        return mysqli_insert_id(self::$link);
    ...
    */
    ...
    {
        $databaseCreds=array();
        $databaseCreds['host']='127.0.0.1';
        $databaseCreds['user']='workflow_cbm3384';
        $databaseCreds['password']='%z?Sr^=X4RwM';
        $databaseCreds['dbname']='workflow_performance';

        $link = @mysqli_connect($databaseCreds['host'], $databaseCreds['user'], $databaseCreds['password']);
        //select db
        mysqli_select_db($databaseCreds['dbname'], $link);
          
        $results = mysqli_query($sql, $link)or mysqli_error();    
