      ...
      <tr>
        <td>
          <form style='margin-bottom:0;' name='form'>
            ...
            <input type='button' value='Search' onclick="filterList('<?=AJAX_DIR?>/balance/get_timerweek.php?id='+document.getElementById('customer').value, 'schedulecontainer');">
            <input type='button' value='Clear' onclick="resetFilterListInputs(); filterList('<?=AJAX_DIR?>/balance/get_timerweek.php', 'schedulecontainer');">
          </form>
        </td>
        <td style="text-align: right;">
        	<a class="btn btn-small btn-success" href="<?=ROOT_DIR?>/payment_history.php"  title="View Payment History" tooltip>Payment History</a>
        </td>
      </tr>
    </table>
<?php
      ...
      <tr>
        <td>
          ...
          <script type='text/javascript'>
            Request.make('<?=AJAX_DIR?>/balance/get_timerweek.php?ws=<?=RequestUtil::get('ws')?>', 'schedulecontainer', true, true);
          </script>
        </td>
      </tr>
    </table>
<?php
    ...
    echo ModuleUtil::showInsufficientRightsAlert('view_balance', TRUE);
}
?>
  </body>
</html>
