...
(function ($) {
    ...
    $.validationEngine = {
        ...
        validateCall: function (caller, rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            function _selectval(caller, rules, position) {   	  // VALIDATE Default value
                defaVal = rules[position + 1];
                curVal = $(caller).val();
                customRule2 = rules[position + 2];
                if (defaVal == curVal || curVal == ' ' || curVal == '') {
                    $.validationEngine.isError = true;
                    promptText += defaVal + "<br />";
                }
            }
        ...
        buildPrompt: function (caller, promptText, type, ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
            ...
            /*Insert New Start*/
            calculatedPosition.callerTopPosition = calculatedPosition.callerTopPosition + 10;
            /*Insert New End*/
        ...
        calculatePosition: function (caller, promptText, type, ajaxed, divFormError) {
            ...
            if ($.validationEngine.settings.promptPosition == "topRight") {
                if ($.validationEngine.settings.containerOverflow) {		// Is the form contained in an overflown container?
                    callerleftPosition += callerWidth - 140;          // dt: 01/09/2015 change validation position older width value is : 30
                } else {
                    callerleftPosition += callerWidth - 140;          // dt: 01/09/2015 change validation position older width value is : 30
